...
router.get('/oneClient', async (req, res) => {
  try {
    const activeUser = User.findById(req.user._id).catch(err => {
      console.log(err)
  ...
  try {
    ...
    const redirectFlow = await allClients.redirectFlows.create({
      ...
      success_redirect_url:
        'https://wholesale-portal-testing.herokuapp.com/cart',
        ...
        async (err, address) => {
          if (err !== null) {
            res.status(400).send({
              errors: {
                ClientAddr1:
                  'Invalid Address. The problem may be also your postal code, state, city'
              }
            })
            return
          }
      ...
      .then(payment => {
        newOrder
          ...
          .then(() => {
            ...
            User.updateOne({ _id: req.user._id }, { cart: [] }).catch(err =>
