...
router.post('/addProduct/:id', async (req, res) => {
        Order.findById(req.params.id).then(order =>
        {
            ...
            const alreadyInOrder = products.findIndex(product => product.product.toString() === req.body.productID);
            if (alreadyInOrder >= 0) {
            ...
            order.updateOne({ products: products })
...
// @access  Private
router.delete('/deleteInQuantity/:id', async (req, res) => {
    Order.findById(req.params.id)
        .then(order => {
            let products = order.products;
            ...
            if (alreadyInOrder >= 0) {
                if (products[alreadyInOrder].quantity > 1)
                    ...
                    products = products.filter(product => { return (product.product != req.body.productID)})
                }
            } else {
...
// @access  Private
router.delete('/deleteWholeProduct/:id', async (req, res) => {
    Order.findById(req.params.id)
         .then(order => {
            let products = order.products;
